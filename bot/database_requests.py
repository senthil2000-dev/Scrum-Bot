...
async def add_scrum_entry(message_id, message, author, tags) -> bool:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(POST_URL, json=scrum_entry) as response:
            if response.status >= 400:
                return False
            response_body = await response.json()
...
async def add_reply(message_id, content, author, parent_message_id) -> bool:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(POST_URL, json=add_reply_body) as response:
            if response.status >= 400:
                return False
            response_body = await response.json()
            return response_body['code'] == 200
...
async def delete_message(message_id) -> bool:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.delete(DELETE_URL, json={'messageId': message_id}) as response:
            if response.status >= 400:
                return False
            response_body = await response.json()
            return response_body['code'] == 200
...
async def update_message(message_id, content, tags=None) -> bool:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.put(PUT_URL, json=update_message_body) as response:
            if response.status >= 400:
                return False
            response_body = await response.json()
            return response_body['code'] == 200
