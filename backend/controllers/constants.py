...
def initConstants():
    ...
    if(len(CONSTANTS) != len(CONSTANT_DEFAULT_VALUES)):
        ...
        errorStr = "Invalid CONSTANTS and CONSTANT_DEFAULT_VALUE array provided. \
            CONSTANTS : {}, CONSTANT_DEFAULT_VALUE: {}".format(CONSTANTS, CONSTANT_DEFAULT_VALUES)
    ...
    for uninitializedConstant in const_array:
        ...
        logging.debug("Creating a new config {} with the value\
                {}".format(uninitializedConstant, CONSTANT_DEFAULT_VALUES[index]))
        newConstant = Constant(name=uninitializedConstant, value=CONSTANT_DEFAULT_VALUES[index])
        newConstant.save()
