...
def getMemberWithGivenId(id:Union[str,ObjectId], **kwargs):
    ...
    try:
        ...
        return parseControllerResponse(data=(MemberInDBSchema(**memberHelper(user))).dict(exclude={"mongoDocument"}),
            statuscode=200, message="Successfully found the user")
        ...
        if isResponseParsed:
            return parseControllerResponse(data=None, statuscode=404, 
                message="User not found", error="A user with id={} does not exist".format(id))
        return None

    except Exception as e:
        helpfulErrorMsg = "Couldn't find a user with the userId {}, due to {}".format(id, e)
